...
# set up the run parameters
  ...
  if(0) {
      k = bio.taxonomy::taxonomy.recode( from="taxa", to="parsimonious", tolookup="wolffish" )
      bio.taxonomy::taxonomy.recode( from="spec", to="taxa", tolookup=k[[1]]$spec )
    #   spec    tsn                      tx                      vern tx_index
    # 1   50 171341        ANARHICHAS LUPUS STRIPED ATLANTIC WOLFFISH     1623
    # 2   51 171342        ANARHICHAS MINOR          SPOTTED WOLFFISH     1624
    # 3   52 550561 ANARHICHAS DENTICULATUS         NORTHERN WOLFFISH     1853
    # 4   59 171335          ANARHICHADIDAE          WOLFFISH UNIDENT     1622
  }
 
  groundfish_survey_species_code = 50 #  STRIPED ATLANTIC WOLFFISH
  groundfish_survey_species_code = 51 #  SPOTTED WOLFFISH
  groundfish_survey_species_code = 52 #  NORTHERN WOLFFISH
  ...
  p = survey_parameters(
    ...
    label ="Northern Wolffish summer",
    speciesname = "Northern_Wolffish",
  ...
  mf = c( "stranal_design", "tesselation" ) [1]
  ...
  if (mf == "stranal_design") {
   ...
   redo_survey_data = FALSE
    ...
    M = survey_db( p=p, DS="carstm_inputs", sppoly=sppoly, redo=redo_survey_data, quantile_upper_limit=0.99, 
      fn=file.path( p$modeldir, p$speciesname , "carstm_inputs_stratanal.rdata" ) )
...
#          + f( cyclic, model="rw2", scale.model=TRUE, hyper=H$rw2, cyclic=TRUE, values=cyclic_values ) 
  ...
  plot( ppa[["mean"]] ~ RES$yr, lty=1, lwd=2.5, col="slategray", type="b", main=mf, ylab="Probability", xlab="Year", ylim=c(0,0.5), pch=19  )
