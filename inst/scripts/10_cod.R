...
# ------------------------------------------------
## Part 2: mimic stranal with aegis.survey::survey_db with inla .. 
## large areal units so environmental covariates do not make sense .. just a simple random effects models
...
# size model
fit = NULL; gc()
fit = carstm_model( p=pW, data=M[iq,], sppoly=sppoly,  posterior_simulations_to_retain="predictions", 
...
# size model
fit = NULL; gc()
fit = carstm_model( p=pW[iq,], data=M, sppoly=sppoly,  posterior_simulations_to_retain="predictions", 
...
# numerical model
fit = NULL; gc()
fit = carstm_model( p=pN[iq,], data=M, sppoly=sppoly,  posterior_simulations_to_retain="predictions", 
